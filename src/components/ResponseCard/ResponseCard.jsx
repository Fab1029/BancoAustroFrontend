...
import { getEmbedding, postEmbedding } from '../../services/Embedding'
import { getLLMResponse, postLLMResponse } from '../../services/LLMResponse'
...
const ResponseCard = ({promt}) => {
    ...
    useEffect(() => {
        ...
        const timer = setTimeout(() => {
            setResponse(promt); // ahora sí se muestra la respuesta
        }, 5000); // espera 5 segundo (puedes ajustarlo)

        return () => clearTimeout(timer);
    }, []);

    // Función para manejar el estado del feedbackcard
    const handleFeedBackOpen = () => {
        if (feedBackOn === false) setFeedBackOn(!feedBackOn);
    };

    // REVISAR
    const handleFeedBackClose = () => {
        // Borrar texto al pulsa botón de cerrar feedback
        const input = document.querySelector('input-feedback');
    ...
    const handleFeedBack = (feedback) => {
        // El feedback dado por el usuario es guardado en la API
        response.retroalimentacion = feedback;
        Number(localStorage.getItem('option')) === 0 ? postLLMResponse(response) : postEmbedding(response);
    }
