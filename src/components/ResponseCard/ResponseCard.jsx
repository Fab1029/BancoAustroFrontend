...
import { getChatResponse } from '../../services/ChatResponse.js'
...
const ResponseCard = ({promt, context, onSumit}) => {
    ...
    useEffect(() => {
        const fetchResponse = async () => {
            ...
            switch (option) {
                case 0:
                    data = await getChatResponse({ consulta: promt, respuesta: '', contexto: context });
                    break;
                case 1:
                    data = await getLLMResponse(promt);
                    break;
                case 2:
                    data = await getEmbedding(promt);
                    break;
            }
    ...
    const handlePostFeedBack = async(feedback) => {
        ...
        switch (option) {
            case 1:
                data = await postLLMResponse(response);
                break;
            case 2:
                data = await postEmbedding(response);
                break;
        }
        ...
        <div className='response-card'>
            <img className='img-banco-austro' src='banco-austro-logo2.png'/>
            {
                ...
                <>
                    ...
                    <div className={`feedback-open-container ${Number(sessionStorage.getItem('option')) !== 0 ? 'show' : ''}`}>
